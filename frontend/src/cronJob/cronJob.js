...
const sendDatatoApi =async()=>{
  ...
  if(myAmount>0){
  try {
  const response = await axios.post('https://prediction-auto-bot.herokuapp.com/api/startBatting', {
    bol:selectedBool,
  ...
  return (
    <div className="container mainContainer">
      <div className="row d-flex justify-content-center align-items-center">
        <div className="col-4 ">
          <select  onChange={(e)=>getMethod(e)} className="form-select myDropDowns" aria-label="Default select example">
      ...
      <div className="row d-flex justify-content-center">
        ...
        <select onChange={(e)=>getBool(e)} className="form-select"   aria-label="Default  select example">
      ...
      <div className="row d-flex justify-content-center">
        <div className="col-4 mt-2">
          <button onClick={()=>sendDatatoApi()}  className="myConfirmButton mt-5">Confirm</button>
        </div>
      </div>
    </div>
  );
